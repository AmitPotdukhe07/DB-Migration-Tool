...
export const ShowFunctions = () => {
    ...
    useEffect(() => {
        setLoader(true)
        customAxios.get("/get-functions")
            .then((response) => {
                if (response.data.success) {
                    setLoader(false);
                    setProcedures(response.data.functions);
                    setProcedureName(response.data.functionName);
                } else {
                    setLoader(false);
                    Swal.fire({
                        position: "center",
    ...
    const handleChange = (event) => {
        ...
        const def = procedures.find((item) => item.FUNCTION_NAME === value[value.length - 1]);
