...
export const ShowViews = () => {
    ...
    useEffect(() => {
        setLoader(true)
        customAxios.get("/get-views")
            .then((response) => {
                setLoader(false)

                if (response.data.success) {
                    setProcedures(response.data.views);
                    setProcedureName(response.data.viewNames);
                } else {
                    setLoader(false)
                    Swal.fire({
                        position: "center",
    ...
    const handleChange = (event) => {
        ...
        const def = procedures.find((item) => item.VIEW_NAME === value[value.length - 1]);
